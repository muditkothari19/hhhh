...
def process_pdfs(pdf_paths):
    queries = [
        ...
        'only the name of coverages and only their limits of liability .',
    ...
    short_names = {
        ...
        'only the name of coverages and only their limits of liability .': 'Coverage & Limits',
...
def save_to_word(results_dict, short_names):
    output = io.BytesIO()
    document = Document()
    document.add_heading('PDF Comparing Results', level=1)
...
def save_to_word_and_prompt(results_dict, short_names):
    ...
    st.download_button(
        ...
        file_name="PDF_Comparing_Results.docx",
        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    )

# Streamlit interface
st.title("PDF Comparing")
...
if uploaded_files:
    ...
    results = process_pdfs(pdf_paths)
